...
exports.list2125Capabilities = list2125Capabilities;
exports.invoke2125 = invoke2125;
...
const CAPABILITY_META = {
    ...
    neural_bookmark: {
        ...
        instruction: 'Analyze content and return JSON: {"title","memoryTags":[],"recallTrigger","longTermWeight":0-1,"summary"}',
        maxTokens: 280,
    },
    emotion_signature: {
        title: 'Emotion Signature',
        instruction: 'Return JSON: {"primaryEmotion","secondaryEmotions":[],"hue":"#hex","intensity":0-1,"poeticLine"} for this post.',
        maxTokens: 200,
    },
    temporal_echo: {
        title: 'Temporal Echo',
        instruction: 'Predict when this memory should resurface. Return JSON: {"resurfaceInDays":number,"reason","lifeEventMatch"}',
        maxTokens: 220,
    },
    vibe_match: {
        title: 'Vibe Match',
        instruction: 'Analyze resonance between two personas. Return JSON: {"score":0-100,"sharedFrequencies":[],"icebreaker"}',
        maxTokens: 260,
    },
    intent_engine: {
        title: 'Intent Engine',
        instruction: 'Predict what the user needs before they search. Return JSON: {"intent","confidence":0-1,"suggestedActions":[]}',
        maxTokens: 280,
    },
    reputation_lattice: {
        title: 'Reputation Lattice',
        instruction: 'Summarize trust across communities. Return JSON: {"latticeScore":0-100,"nodes":[],"insight"}',
        maxTokens: 300,
    },
    mood_oracle: {
        title: 'Mood Oracle',
        instruction: 'Read interaction patterns and infer mood. Return JSON: {"detectedMood","visualTheme","feedTone","oracleMessage"}',
        maxTokens: 240,
    },
    twin_digest: {
        title: 'Twin Digest',
    ...
    dream_forge: {
        title: 'Dream Forge',
        instruction: 'Forge a visual journey map. Return JSON: {"phases":[{"name","tasks":[],"durationWeeks":n}],"mantra"}',
        maxTokens: 400,
    },
    spam_shield: {
        title: 'Spam Shield',
...
function list2125Capabilities() {
    return Object.keys(CAPABILITY_META).map((id) => ({
        id,
...
function categorize(id) {
    if (['digital_twin', 'twin_digest', 'mood_oracle', 'vibe_match'].includes(id))
        return 'Consciousness';
    if (['neural_bookmark', 'memory_weaver', 'temporal_echo', 'dream_forge'].includes(id))
        return 'Memory';
    if (['emotion_signature', 'feed_harmonizer', 'caption_prism', 'post_transcend'].includes(id))
        return 'Creation';
    if (['spam_shield', 'toxicity_nullifier', 'reputation_lattice'].includes(id))
        return 'Shield';
    if (['trend_prophet', 'intent_engine', 'smart_search_lens', 'interest_constellation'].includes(id))
        return 'Oracle';
    return 'Studio';
}
function seed(userId, capability, input) {
    const h = (0, crypto_1.createHash)('sha256').update(`${userId}:${capability}:${input}`).digest();
    return h.readUInt32BE(0);
}
function fallbackResult(capability, input, userId) {
...
async function invoke2125(userId, capability, input) {
    const meta = CAPABILITY_META[capability];
    const userText = [
        ...
        .filter(Boolean)
        .join('\n');
    if (!(0, openai_client_1.getOpenAI)()) {
        return fallbackResult(capability, input, userId);
    }
    const { lastResponses } = await (0, anti_repetition_engine_1.getAiContext)(userId, capability);
    const avoid = lastResponses.length ? `Do not repeat ideas from: ${lastResponses.slice(-3).join(' | ')}` : '';
    try {
        let output = await (0, openai_client_1.chatCompletion)([
            { role: 'system', content: `${VYRA_PERSONA}\n${meta.instruction}\n${avoid}` },
            { role: 'user', content: userText || 'Generate insight for the user.' },
        ], { temperature: (0, anti_repetition_engine_1.taskTemperature)(capability), maxTokens: meta.maxTokens });
        if ((0, anti_repetition_engine_1.isTooSimilar)(output, lastResponses)) {
            output = `${output}\n\n— signal refracted at ${new Date().toISOString()}`;
        }
        await (0, anti_repetition_engine_1.saveAiResponse)(userId, capability, userText, output);
        let structured;
        try {
        ...
        return {
            ...
            structured,
            mood: input.mood,
            energySignature: `VYRA-${(0, crypto_1.createHash)('md5').update(output).digest('hex').slice(0, 8).toUpperCase()}`,
            poweredBy: 'openai',
        };
    }
    catch {
...
//# sourceMappingURL=vyra-2125.engine.js.map